import{a as u}from"./ids-chunk-G4EPNT7L.js";import{c as s}from"./ids-chunk-LCUXCSNZ.js";import{a as c,b as p}from"./ids-chunk-XE4WFW5M.js";import{a as r}from"./ids-chunk-EQJWBFQ2.js";import{a as l}from"./ids-chunk-U7OOVFXE.js";var h=`
@charset "UTF-8";
/**
...
:host ids-menu-button {
  transition: border var(--ids-motion-duration) var(--ids-motion-ease);
  border: 0;
  border-inline: 0;
  border-inline-end: 1px solid var(--ids-input-color-border-default);
  height: 100%;
  display: flex;
  flex-direction: row;
...
:host ids-menu-button::part(button) {
  border: 0;
  border-radius: 0;
  margin: 0;
  margin-inline: 0;
...
:host ids-menu-button::part(button), :host ids-menu-button::part(button):active, :host ids-menu-button::part(button):hover {
  background: transparent;
  background-color: transparent;
}

:host(:focus-within) ids-menu-button,
:host(:focus-visible) ids-menu-button {
  border-color: var(--ids-input-color-border-focus);
}
:host(:focus-within) ids-menu-button::part(button),
:host(:focus-visible) ids-menu-button::part(button) {
  color: var(--ids-button-tertiary-color-text-hover);
}

:host(:active) ids-menu-button,
:host(:hover) ids-menu-button {
  border-color: var(--ids-input-color-border-hover);
}

.ids-search-field ids-icon.search-icon {
  margin-inline-start: var(--ids-search-field-icon-margin);
}
.ids-search-field .fieldset {
  display: inline-flex;
  flex-direction: row;
}
.ids-search-field #category-action-button::part(button) {
  padding: var(--ids-space-40);
  margin-inline-start: var(--ids-search-field-icon-margin);
}
.ids-search-field.color-variant-alternate .field-container {
  border-style: none;
  background-color: var(--ids-search-field-header-color-background);
  color: var(--ids-search-field-header-color-text);
}
.ids-search-field.color-variant-alternate ids-icon {
  color: var(--ids-search-field-header-color-icon-default);
}
.ids-search-field.color-variant-card .field-container {
  background: var(--ids-search-field-card-color-background);
  color: var(--ids-search-field-card-color-text);
}
.ids-search-field.color-variant-card .btn-clear:hover {
  background-color: transparent;
}
.ids-search-field.color-variant-app-menu {
  color: var(--ids-search-field-app-menu-color-text);
}
.ids-search-field.color-variant-app-menu .field-container {
  border-style: none;
  border-radius: var(--ids-search-field-border-radius);
  background-color: var(--ids-search-field-app-menu-color-background);
  color: var(--ids-search-field-app-menu-color-text);
}
.ids-search-field.color-variant-app-menu .ids-input-field {
  color: var(--ids-search-field-app-menu-color-text);
}
.ids-search-field.color-variant-app-menu .btn-clear {
  color: var(--ids-search-field-app-menu-color-icon-default);
}
.ids-search-field.color-variant-app-menu .btn-clear:hover {
  color: var(--ids-search-field-app-menu-color-icon-hover);
}
.ids-search-field.color-variant-app-menu ids-icon::part(search-icon) {
  color: var(--ids-search-field-app-menu-color-text);
}
.ids-search-field.color-variant-app-menu:focus-within ids-icon::part(search-icon) {
  color: var(--ids-search-field-app-menu-color-icon-hover);
}
.ids-search-field.color-variant-app-menu:focus-within .field-container, .ids-search-field.color-variant-app-menu.readonly:focus-within .field-container {
  box-shadow: var(--ids-search-field-app-menu-shadow-focus);
}`;var g="Search",m="Type to search",n=class extends u{constructor(){super()}isFormComponent=!0;colorVariants=["alternate","app-menu","card"];onColorVariantRefresh(i){let t=i||"";i==="app-menu"&&(t="alternate");let e=o=>{o.setAttribute(r.COLOR_VARIANT,t)};this.buttons.forEach(e),this.fieldContainer&&[...this.fieldContainer.querySelectorAll("ids-trigger-button")].forEach(e)}static get attributes(){return[...super.attributes,r.CATEGORY]}connectedCallback(){super.connectedCallback(),this.#s(),this.#l(),this.placeholder||(this.placeholder=m),this.label||(this.label=g)}template(){this.templateHostAttributes();let{ariaLabel:i,containerClass:t,inputClass:e,inputState:o,labelHtml:a,placeholder:d,type:v,value:b}=this.templateVariables(),f='<ids-icon class="ids-icon search-icon starting-icon" size="medium" icon="search"></ids-icon>';return`<div id="ids-search-field" class="ids-search-field ids-trigger-field ${t}" part="container">
      ${a}
      <div class="fieldset">
        <div class="field-container" part="field-container">
          ${this.categories.length?"":f}
    ...
    </div>`}templateCategoriesButton(){return this.action?`
      <ids-button id="category-action-button" appearance="secondary">
        <span>${this.action}</span>
      </ids-button>
    `:""}templateCategoriesMenu(){return this.categories.length?`
      <ids-menu-button id="menu-button" appearance="tertiary" menu="category-menu" icon="search" dropdown-icon>
        ${this.category?`<span>${this.category}</span>`:'<span class="audible">Select Search Category</span>'}
      </ids-menu-button>
      <ids-popup-menu id="category-menu" target="menu-button" trigger-type="click" align="bottom, right">
        <ids-menu-group select="${this.multiple?"multiple":"single"}" keep-open="true">
          ${this.categories.map((t,e)=>`<ids-menu-item value="${e}">${t}</ids-menu-item>`).join("")}
        </ids-menu-group>
      </ids-popup-menu>
    `:""}#o(){let i=this.category,t=this.#n;if(!i||!t)return;let e=this.selectedCategories;e.length===1?t.text=e[0]:e.length>1?t.text=`${e.length} ${this.localeAPI?.translate?.("Selected")}`:t.text=i;let o=this.#i?.popup,a=parseInt(o?.style?.getProperty?.("min-width")??155),d=this.#n?.getBoundingClientRect().width??a;o?.style?.setProperty?.("min-width",`${Math.max(d,a)}px`),this.multiple||this.#i?.hide?.()}get#i(){return this.shadowRoot?.querySelector("ids-popup-menu")}get#n(){return this.shadowRoot?.querySelector("ids-menu-button")}get#a(){return this.shadowRoot?.querySelector("ids-button#category-action-button")}#d=[];get categories(){return this.#d}set categories(i){this.#d=i,this.#r()}set category(i){i?this.setAttribute(r.CATEGORY,i):this.removeAttribute(r.CATEGORY),this.#r()}get category(){return this.getAttribute(r.CATEGORY)??""}get selectedCategories(){let i=this.categories;return(this.#i?.getSelectedValues?.()??[]).map(o=>i[o])}set action(i){i?this.setAttribute(r.ACTION,i):this.removeAttribute(r.ACTION),this.#r()}get action(){return this.getAttribute(r.ACTION)??""}set multiple(i){i?this.setAttribute(r.MULTIPLE,""):this.removeAttribute(r.MULTIPLE)}get multiple(){return this.hasAttribute(r.MULTIPLE)}async search(i){let t=[],e=s(i);return this.#e!==e&&(this.input&&(this.input.value=e),this.#e=e,typeof this.onSearch=="function"&&(t=await this.onSearch(e))),t}set value(i){super.value=i;let t=super.value;this.#e=t,typeof this.onSearch=="function"&&this.onSearch(t)}get value(){return super.value}onSearch(i=void 0){return[`${s(i)}`]}#e=null;#s(){let i=t=>{this.search(t.target?.value)};this.offEvent("change",this.input),this.onEvent("change",this.input,i),this.offEvent("input",this.input),this.onEvent("input",this.input,i),this.offEvent("click",this.#a),this.onEvent("click",this.#a,()=>{this.search(this.value),this.#t("search")}),this.offEvent("selected",this.#i),this.onEvent("selected",this.#i,()=>{this.#o(),this.#t("selected")}),this.offEvent("deselected",this.#i),this.onEvent("deselected",this.#i,()=>{this.#o(),this.#t("deselected")})}#l(){this.offEvent("keydown",this.input),this.onEvent("keydown",this.input,i=>{let t=!i?.path?.length||!i.path[0].classList||!i.path[0].classList.contains("ids-icon-button");switch(["Enter"].indexOf(i.code)>-1&&t&&i.preventDefault(),i.key){case"Enter":t&&(this.onSearch(this.input?.value),this.#t("search"));break;default:break}})}#t(i){this.triggerEvent(i,this,{detail:{elem:this,categories:this.categories,categoriesSelected:this.selectedCategories,value:this.value},bubbles:!0,cancelable:!1,composed:!1})}#r(){this.container&&(this.container.innerHTML=this.template(),this.connectedCallback())}};n=l([c("ids-search-field"),p(h)],n);export{n as a};
