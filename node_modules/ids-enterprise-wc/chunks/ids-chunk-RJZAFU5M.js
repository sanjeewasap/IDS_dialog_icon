import{a as v}from"./ids-chunk-G4EPNT7L.js";import{a as b,b as h}from"./ids-chunk-XE4WFW5M.js";import{e as n}from"./ids-chunk-QPDOZZIG.js";import{a as i,b as o}from"./ids-chunk-EQJWBFQ2.js";import{a as p}from"./ids-chunk-U7OOVFXE.js";var f=`
@charset "UTF-8";
/**
...
.ids-spinbox.readonly ids-button::part(button) {
...
.ids-spinbox.disabled ids-input::part(input) {
  border-color: var(--ids-spinbox-color-border-disabled);
  color: var(--ids-spinbox-color-text-disabled);
  pointer-events: none;
}
.ids-spinbox.disabled ids-button::part(button) {
  color: var(--ids-spinbox-color-text-disabled);
}
...
.ids-spinbox .ids-spinbox-content {
  contain: content;
  display: flex;
  flex-direction: row;
  width: fit-content;
...
.ids-spinbox .ids-spinbox-content:focus-within ids-input:not(.error)::part(input) {
  border-color: var(--ids-spinbox-color-border-focus);
  border-width: 1px;
  box-shadow: none;
}
.ids-spinbox .ids-spinbox-content:focus-visible ids-button::part(button) {
  outline: none;
}`;var y=0,w=1,l=class extends v{constructor(){super()}isFormComponent=!0;static get attributes(){return[...super.attributes,i.MAX,i.MIN,i.STEP]}template(){this.templateHostAttributes();let{ariaLabel:t,containerClass:r,inputClass:e,inputState:a,labelHtml:s,placeholder:d,type:c,value:u}=this.templateVariables();return`<div id="ids-spinbox" class="ids-spinbox ids-trigger-field ${r}" part="container">
        ${s}
      ...
      </div>`}templateHostAttributes(){this.id||this.setAttribute(i.ID,`ids-spinbox-${++y}`)}#s(){this.querySelector('[slot="trigger-start"]')||this.insertAdjacentHTML("afterbegin",`<ids-trigger-button
        ...
        slot="trigger-start"
        tabbable="false"
        part="button"> - </ids-trigger-button>`),this.querySelector('[slot="trigger-end"]')||this.insertAdjacentHTML("beforeend",`<ids-trigger-button
        ...
        slot="trigger-end"
        tabbable="false"
        part="button"> + </ids-trigger-button>`)}connectedCallback(){super.connectedCallback(),this.setAttribute(o.ROLE,"spinbutton"),this.setAttribute(o.ARIA_VALUENOW,this.value),this.setAttribute(o.ARIA_LABEL,this.label),n(this.getAttribute(i.MAX))&&this.setAttribute(o.ARIA_VALUEMAX,String(this.max)),n(this.getAttribute(i.MIN))&&this.setAttribute(o.ARIA_VALUEMIN,String(this.min)),this.#s(),n(this.getAttribute(i.TABBABLE))||(this.tabbable=!1),n(this.getAttribute(i.SIZE))||(this.size="sm"),this.hasAttribute("text-align")||(this.textAlign="center"),this.#a(),this.#d(),this.#t()}#d(){this.input?.addEventListener("change",()=>{this.input&&this.input.value!==this.value&&(this.value=this.input.value,this.#r())}),this.onEvent("mousedown.increment",this.incrementButton,this.#n("up")),this.onEvent("mousedown.decrement",this.decrementButton,this.#n("down")),this.onEvent("mouseup",window,t=>{this.#r(t)}),this.onEvent("focus",this,t=>{this.hasAttribute(i.DISABLED)||(t.preventDefault(),this.focus())}),this.listen(["ArrowUp","ArrowDown"],this,t=>{if(n(this.getAttribute(i.DISABLED)))return;let r=t.key;switch(this.#r(),r){case"ArrowUp":this.#o("up");break;case"ArrowDown":this.#o("down");break;default:}t.preventDefault()})}get decrementButton(){return this.querySelector('ids-trigger-button[slot="trigger-start"]')}get incrementButton(){return this.querySelector('ids-trigger-button[slot="trigger-end"]')}set max(t){if(this.getAttribute(i.MAX)!==t){let e=parseInt(t);Number.isNaN(e)?(this.removeAttribute(i.MAX),this.removeAttribute(o.ARIA_VALUEMAX)):(this.setAttribute(i.MAX,`${e}`),this.setAttribute(o.ARIA_VALUEMAX,`${e}`)),this.#a(),this.#t()}}get max(){return this.hasAttribute(i.MAX)?parseInt(this.getAttribute(i.MAX)):null}set min(t){if(this.getAttribute(i.MIN)!==t){let e=parseInt(t);Number.isNaN(e)?(this.removeAttribute(i.MIN),this.removeAttribute(o.ARIA_VALUEMIN)):(this.setAttribute(i.MIN,`${e}`),this.setAttribute(o.ARIA_VALUEMIN,`${e}`)),this.#t()}}get min(){return this.hasAttribute(i.MIN)?parseInt(this.getAttribute(i.MIN)):null}set step(t){if(this.getAttribute(i.STEP)!==t){let e=parseInt(t);(Number.isNaN(e)||e<1)&&(e=1),this.setAttribute(i.STEP,`${e}`),this.#t()}}get step(){return this.hasAttribute(i.STEP)?parseInt(this.getAttribute(i.STEP)):1}set tabbable(t){let r=n(t);this.setAttribute(i.TABBABLE,String(r))}get tabbable(){return n(this.getAttribute(i.TABBABLE))}set value(t){if(this.input&&`${t}`.trim()==="-"){this.input.value="";return}let r=super.value==="",e=`${parseInt(t)}`;e!=="NaN"&&super.value!==e&&(super.value=this.#l(e),this.setAttribute(o.ARIA_VALUENOW,super.value),this.#t(),r&&this.validate&&this.checkValidation())}get value(){return super.value}#l(t){let r=parseInt(t);if(!Number.isNaN(r)){let e=parseInt(this.step);!Number.isNaN(e)&&r%e!==0&&(r=Math.round(r/e)*e);let s=this.min,d=this.max,c=typeof this.min=="number"&&!Number.isNaN(this.min),u=typeof this.max=="number"&&this.max&&!Number.isNaN(this.max),g=c&&r<s,m=u&&r>d,x=u&&r===d||c&&r===s;g&&(r=Math.max(r,s)),m&&(r=Math.min(r,d)),x&&this.#r(),r=`${r}`}return r}set disabled(t){let r=n(t);super.disabled=r,r?(this.incrementButton?.setAttribute?.(i.DISABLED,""),this.decrementButton?.setAttribute?.(i.DISABLED,"")):this.hasAttribute(i.READONLY)||(this.incrementButton?.removeAttribute?.(i.DISABLED),this.decrementButton?.removeAttribute?.(i.DISABLED))}get disabled(){return super.disabled}set readonly(t){let r=n(t);super.readonly=r,r?(this.#r(),this.incrementButton?.setAttribute(i.DISABLED,""),this.decrementButton?.setAttribute(i.DISABLED,"")):this.hasAttribute(i.DISABLED)||(this.incrementButton?.removeAttribute(i.DISABLED),this.decrementButton?.removeAttribute(i.DISABLED))}get readonly(){return super.readonly}#o(t){let e=!Number.isNaN(parseInt(this.step))?parseInt(this.step):1;t==="down"&&(e*=-1);let a=!Number.isNaN(parseInt(this.value));this.value=`${(a?parseInt(this.value):0)+e}`}#t(){if(!!Number.isNaN(parseInt(this.max))){this.incrementButton?.removeAttribute(i.DISABLED);return}if(parseInt(this.value)>=parseInt(this.max)?this.incrementButton?.setAttribute(i.DISABLED,""):this.hasAttribute(i.READONLY)||this.incrementButton?.removeAttribute(i.DISABLED),!!Number.isNaN(parseInt(this.min))){this.decrementButton?.removeAttribute(i.DISABLED);return}parseInt(this.value)<=parseInt(this.min)?this.decrementButton?.setAttribute(i.DISABLED,""):this.hasAttribute(i.READONLY)||this.decrementButton?.removeAttribute(i.DISABLED),this.disabled&&(this.incrementButton?.setAttribute(i.DISABLED,""),this.decrementButton?.setAttribute(i.DISABLED,""))}#e;#i;#n(t){return r=>{if(w&r.buttons){let e=0;this.#e=t;let a=()=>{this.#e===t?(this.#o(t),e++,this.#i=setTimeout(a,Math.max(350-Math.round(e*50),100))):(clearTimeout(a),this.#i=void 0)};this.#i=a,this.#i()}}}#r(t){(!t||t.which===1&&(this.#i||this.#e))&&(clearInterval(this.#i),this.#i=void 0,this.#e=void 0)}#a(){let t={allowDecimal:!1,allowNegative:!0};this.mask="number",this.maskOptions=t}};l=p([b("ids-spinbox"),h(f)],l);export{l as a};
