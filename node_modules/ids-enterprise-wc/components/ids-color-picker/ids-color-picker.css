...
.ids-color-picker .color-preview {
  margin-inline-start: 0;
}
.ids-color-picker input {
  text-overflow: ellipsis;
}
.ids-color-picker ids-trigger-field::part(dirty-tracker-icon) {
  margin-inline-start: 35px;
}
.ids-color-picker.full {
  max-width: var(--ids-input-width-full);
}
...
.color-picker-trigger-btn::part(button) {
  margin-inline-start: 1px;
